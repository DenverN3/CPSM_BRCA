...
download_and_prepare_tcga_brca <- function() {
  cat("🎗️ DOWNLOADING TCGA-BRCA (BREAST CANCER) DATA\n")
  cat("==============================================\n")
  ...
  cat("\n🔄 CONVERTING TO CPSM FORMAT - BREAST CANCER SPECIFICS\n")
  cat("======================================================\n")
...
run_brca_cpsm_pipeline <- function(combined_df, split_ratio = 0.8) {
  cat("\n🔬 RUNNING BREAST CANCER CPSM PIPELINE\n")
  cat("======================================\n")
  ...
  tryCatch({
    ...
    if (test_events >= 20) {
      cat("✅ Excellent: Sufficient events for reliable C-index calculation\n")
    } else {
      cat("⚠️ Warning: Limited events may affect C-index reliability\n")
    }
  ...
  cat("\n🧬 Step 4: Breast Cancer Gene Analysis\n")
  cat("--------------------------------------\n")
  tryCatch({
  ...
  cat("\n🏗️ Step 5: Breast Cancer Model Development\n")
  cat("------------------------------------------\n")
...
perform_brca_univariate_selection <- function(results) {
  ...
  cat(sprintf("\n🎯 Found %d breast cancer survival-associated genes (p < 0.05)\n", 
...
develop_brca_models <- function(results) {
  ...
  cat("\n🏥 Model 1: Breast Cancer Clinical Features\n")
  ...
  tryCatch({
    ...
    cat("✅ Breast Cancer Clinical Model Performance:\n")
    ...
    cat("❌ Clinical model error:", e$message, "\n")
    results$Model1 <- NULL
  })
  
  # Model 2: Gene Signature Model
  if (length(results$significant_genes) > 0) {
    cat("\n🧬 Model 2: Breast Cancer Gene Signature\n")
    ...
    tryCatch({
      ...
      cat("✅ Gene Signature Model Performance:\n")
      ...
      cat("❌ Gene model error:", e$message, "\n")
      results$Model2 <- NULL
    })
  }
  ...
  if (length(results$significant_genes) > 0) {
    ...
    tryCatch({
      ...
      cat("✅ Combined Model Performance:\n")
      ...
      cat("❌ Combined model error:", e$message, "\n")
      results$Model3 <- NULL
    })
  }
  ...
  if (!is.null(best_model)) {
    cat(sprintf("\n🎉 Best breast cancer model: %s (C-index = %.3f)\n", 
...
annotate_brca_genes <- function(results) {
  ...
  tryCatch({
    ...
    cat("📁 Gene annotation saved to 'brca_gene_annotation.csv'\n")
    
    return(gene_annotation)
    
  }, error = function(e) {
    cat("❌ Gene annotation error:", e$message, "\n")
    return(NULL)
  })
}
...
run_tcga_brca_cpsm_analysis <- function(split_ratio = 0.8) {
  cat("🎗️ TCGA BREAST CANCER CPSM ANALYSIS\n")
  cat("===================================\n\n")
  ...
  cat("\n📊 ANALYSIS COMPLETE - BREAST CANCER RESULTS\n")
  cat("============================================\n")
  ...
  if (!is.null(results)) {
    ...
    if (!is.null(results$best_model)) {
      ...
      if (results$best_c_index > 0.7) {
        cat("🎉 EXCELLENT predictive performance for breast cancer!\n")
      } else if (results$best_c_index > 0.6) {
        cat("✅ GOOD predictive performance for breast cancer!\n")
      } else if (results$best_c_index > 0.55) {
        cat("📈 MODERATE predictive performance for breast cancer\n")
      } else {
        cat("📊 BASELINE predictive performance for breast cancer\n")
      }
    }
  }
...
# USAGE INSTRUCTIONS
# ========================================================================
cat("🎗️ BREAST CANCER CPSM PIPELINE - READY!\n")
cat("========================================\n\n")
...
cat("🎯 KEY DIFFERENCES FROM COLORECTAL CANCER:\n")
cat("==========================================\n")
cat("• Target: TCGA-BRCA (Breast Cancer) instead of TCGA-COAD\n")
cat("• Clinical features: ER/PR status, breast cancer staging\n")
cat("• Gene signatures: Breast cancer-specific survival genes\n")
cat("• Larger sample size: ~1000+ breast cancer samples\n")
cat("• Different molecular subtypes and pathways\n\n")

cat("Ready to analyze breast cancer data! 🎗️\n")

# Execute the analysis
cat("🚀 STARTING BREAST CANCER ANALYSIS...\n")
...
create_brca_survival_plots <- function(brca_results) {
  ...
  cat("📊 Creating breast cancer survival visualizations...\n")
  ...
  cat("📁 Breast cancer dashboard saved as 'brca_survival_dashboard.png'\n")
...
compare_cancer_models <- function(brca_results, coad_results = NULL) {
  cat("📊 CANCER TYPE MODEL COMPARISON\n")
  cat("===============================\n")
...
create_gene_signature_heatmap <- function(brca_results, top_n = 20) {
  ...
  cat("🔥 Creating breast cancer gene signature heatmap...\n")
  ...
  if (!is.null(brca_results$Train_Uni_sig_data)) {
    ...
    if (length(gene_cols) > 0) {
      ...
      tryCatch({
        ...
        cat("📁 Gene signature heatmap saved as 'brca_gene_signature_heatmap.png'\n")
...
create_gene_survival_curves <- function(brca_results, gene_name = NULL, top_n = 4) {
  if (is.null(brca_results$significant_genes)) {
    cat("❌ No significant genes available\n")
    return(NULL)
  }
  ...
  cat(sprintf("📈 Creating survival curves for %d breast cancer genes...\n", 
  ...
  cat(sprintf("📁 Saved %d gene survival curves\n", length(plot_list)))
...
generate_brca_report <- function(brca_results) {
  cat("📋 GENERATING COMPREHENSIVE BREAST CANCER REPORT\n")
  cat("================================================\n")
  ...
  report_text <- c(report_text, 
    ...
    "FILES GENERATED:",
    "================",
    "• brca_survival_dashboard.png - Survival analysis dashboard",
    "• brca_gene_signature_heatmap.png - Gene expression heatmap", 
    "• brca_gene_annotation.csv - Gene symbols and annotations",
    "• brca_*_survival.png - Individual gene survival curves",
    "• brca_analysis_report.txt - This summary report"
  )
  
  # Save report
  writeLines(report_text, "brca_analysis_report.txt")
  
  cat("📁 Comprehensive report saved as 'brca_analysis_report.txt'\n")
  cat("📊 All visualizations and data files generated!\n")
...
# USAGE EXAMPLES
# ========================================================================
cat("🎗️ BREAST CANCER VISUALIZATION TOOLS - READY!\n")
cat("==============================================\n\n")
...
cat("🎯 FEATURES:\n")
cat("============\n")
cat("• Breast cancer-specific survival dashboards\n")
cat("• Gene signature heatmaps with survival annotation\n")
cat("• Individual gene survival curves\n") 
cat("• Cancer type performance comparisons\n")
cat("• Comprehensive automated reports\n")
cat("• High-quality publication-ready plots\n\n")

cat("Ready to visualize breast cancer results! 🎗️\n")
